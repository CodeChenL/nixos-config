...
let
  ...
  package = pkgs.natfrp-service;
  user = "natfrp";
  group = "natfrp";
  workDir = "/var/lib/natfrp";
  apiBaseUrl = "https://api.natfrp.com/v4";
  tokenFile = "${workDir}/token";
  remotePasswordFile = "${workDir}/remote-password";
  ...
  generateConfig = pkgs.writeShellScript "natfrp-generate-config" ''
    ...
    install -d -m 0700 -o ${user} -g ${group} ${lib.escapeShellArg workDir}
    ...
    if [ ${startAllTunnelsEnabled} = true ]; then
      ...
      for attempt in 1 2 3 4 5; do
        if tunnels_json=$(${pkgs.curl}/bin/curl -fsSL \
          ...
          ${lib.escapeShellArg "${apiBaseUrl}/tunnels"}); then
          break
        fi
    ...
    if [ ${remoteManagementEnabled} = true ]; then
      remote_password_file=${lib.escapeShellArg remotePasswordFile}
      ...
      remote_key=$(${package}/bin/natfrp-service remote-kdf "$remote_password" | tr -d '\r\n')
    fi
    ...
    install -m 0600 -o ${user} -g ${group} "$tmp" ${lib.escapeShellArg workDir}/config.json
  '';
in
{
  ...
  config = lib.mkIf cfg.enable {
      ...
      text = ''
        ${lib.optionalString (cfg.tokenSourceFile != "") ''
          if [ -f "${cfg.tokenSourceFile}" ]; then
            install -D -m 0600 -o ${user} -g ${group} "${cfg.tokenSourceFile}" "${workDir}/token"
          fi
        ''}
        ${lib.optionalString (cfg.remoteManagement.enable && cfg.remoteManagement.passwordSourceFile != "") ''
          if [ -f "${cfg.remoteManagement.passwordSourceFile}" ]; then
            install -D -m 0600 -o ${user} -g ${group} "${cfg.remoteManagement.passwordSourceFile}" "${workDir}/remote-password"
          fi
        ''}
      '';
      deps = [ "users" ];
    ...
    systemd.services.natfrp = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${package}/bin/natfrp-service --daemon";
        Restart = "always";
        RestartSec = 5;
        TimeoutStopSec = 20;
        Environment = [
          "NATFRP_SERVICE_WD=${workDir}"
          "NATFRP_FRPC_PATH=${package}/bin/frpc"
        ];
      };
    };
  };
